...
def analyze_guid_data(
    ...
    # ── Single RETEST pass: duplicate detection ───────────────────────────────
    _i = 0
    for _wid, _is_pass in zip(rt_wids, rt_pass):
        if _wid:
            if _wid in fp_pass_ids and _wid in wxy_head_pass_ids:
    ...
    for _row, _hit, _wid in zip(retest_rows, _rt_dup_hits, rt_wids):
        _row[_dl] = _row[_sf]
        _row[_dw] = (_id_to_wxy[_wid], _hit) if _hit else "N/A"
        _row["is_duplicate"] = "YES" if _hit else "NO"
        _row["reject_in_retest"] = "N/A"
    ...
    total_good = total_pass_parts - duplicate_guid
    ...
    check2_result = "FAIL" if duplicate_guid > 72 else "PASS"
    ...
    _emit_log("-" * 80, True, logger)
    _emit_log(f"Duplicate GUID: {duplicate_guid:,}", True, logger)
    ...
    _emit_log(f"Total Good = Total PASS parts - Duplicate GUID = {total_good:,}", True, logger)
