...
def analyze_guid_data(
    ...
    _rt_origin_dup_hits = _CArray('i', bytes(4 * _rt_len))

    fp_pass_ids:       Set[int] = set()
    wxy_head_pass_ids: Set[int] = set()
    # Original duplicate counter:
    # Counts duplicates only when the WXY has an established original PASS from FIRST PASS.
    duplicate_guid = 0
    duplicate_wxy_list: List[Tuple[str, int]] = []   # (wxy_string, combined_part_number)

    # New RETEST-origin duplicate counter:
    # Counts duplicates when the WXY did NOT have an original PASS from FIRST PASS,
    # but already had an established original PASS from RETEST.
    # Example counted here: FIRST PASS A FAIL -> 1ST RETEST A PASS -> 2ND RETEST A PASS.
    retest_duplicate_guid = 0
    retest_duplicate_wxy_list: List[Tuple[str, int]] = []
    retest_origin_pass_ids: Set[int] = set()
    ...
    _rt_origin_add = retest_origin_pass_ids.add
    ...
    # ── Single RETEST pass: original FP duplicate + RETEST-origin duplicate detection ──
    _i = 0
    for _wid, _is_pass in zip(rt_wids, rt_pass):
        if _wid:
            _pn = _fp_len + _i + 1
            ...
            # Duplicate only if this WXY has an original PASS from FIRST PASS and
            # the previous chain-head state is PASS.
            if _wid in fp_pass_ids and _wid in wxy_head_pass_ids:
                duplicate_guid += 1
                _dup_app((_id_to_wxy[_wid], _pn))
                _rt_dup_hits[_i] = _pn          # raw C int write — no Python object

            # New RETEST-origin rule:
            # If no FIRST PASS PASS established this WXY as original, allow the first
            # RETEST PASS to become the original. Later RETEST rows are duplicates
            # when the previous RETEST-origin chain-head state is PASS.
            elif _wid in retest_origin_pass_ids and _wid in wxy_head_pass_ids:
                retest_duplicate_guid += 1
                _rt_dup_app((_id_to_wxy[_wid], _pn))
                _rt_origin_dup_hits[_i] = _pn

            if _is_pass:
                _h_add(_wid)
                if _wid not in fp_pass_ids:
                    _rt_origin_add(_wid)
            else:
                _h_dis(_wid)
        _i += 1
    ...
    _dl = "datalog"
    _dw = "duplicate_wxy"
    for _row, _hit, _wid, _is_pass, _hb in zip(fp_rows, _fp_dup_hits, fp_wids, fp_pf, fp_hbin):
        _row[_dl] = _row[_sf]
        _row[_dw] = (_id_to_wxy[_wid], _hit) if _hit else "N/A"
        _row["is_duplicate"] = "YES" if _hit else "NO"
        _row["retest_duplicate_wxy"] = "N/A"
        _row["is_retest_duplicate"] = "NO"
        _row["is_total_duplicate"] = "YES" if _hit else "NO"
        if not _is_pass and _hb != 3 and _wid:
            _row["reject_in_retest"] = "YES" if _wid in retest_all_ids else "NO"
        else:
            _row["reject_in_retest"] = "N/A"
            
    for _row, _hit, _rt_origin_hit, _wid in zip(retest_rows, _rt_dup_hits, _rt_origin_dup_hits, rt_wids):
        _row[_dl] = _row[_sf]
        _row[_dw] = (_id_to_wxy[_wid], _hit) if _hit else "N/A"
        _row["is_duplicate"] = "YES" if _hit else "NO"
        _row["retest_duplicate_wxy"] = (_id_to_wxy[_wid], _rt_origin_hit) if _rt_origin_hit else "N/A"
        _row["is_retest_duplicate"] = "YES" if _rt_origin_hit else "NO"
        _row["is_total_duplicate"] = "YES" if (_hit or _rt_origin_hit) else "NO"
        _row["reject_in_retest"] = "N/A"
    ...
    total_status = "PASS" if total_delta >= 0 else "FAIL"
    ...
    _emit_log("-" * 80, True, logger)
    _emit_log(f"Duplicate GUID - FIRST PASS original rule: {duplicate_guid:,}", True, logger)
    _emit_log("Duplicate GUID rule: a part is a duplicate when its WXY has an established original (first PASS in FIRST PASS) and its chain-head predecessor had a PASS flag, regardless of the duplicate part's own PASS/FAIL flag.", True, logger)
    _emit_log(f"Duplicate GUID - RETEST original rule: {retest_duplicate_guid:,}", True, logger)
    _emit_log("RETEST duplicate GUID rule: when a WXY has no original PASS from FIRST PASS, the first RETEST PASS establishes the original; later RETEST rows are duplicates when the previous RETEST-origin chain-head was PASS, regardless of the duplicate part's own PASS/FAIL flag.", True, logger)
