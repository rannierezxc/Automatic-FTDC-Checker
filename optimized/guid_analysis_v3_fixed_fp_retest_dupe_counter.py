...
def analyze_guid_data(
    ...
    # Counts the first RETEST duplicate against the established FIRST PASS PASS original.
    duplicate_guid = 0
    ...
    # Example A: FIRST PASS A PASS -> 1ST RETEST A PASS -> 2ND RETEST A PASS
    #            duplicate_guid=1, retest_duplicate_guid=1
    # Example B: FIRST PASS A PASS -> 1ST RETEST A PASS -> 2ND RETEST A FAIL
    #            duplicate_guid=1, retest_duplicate_guid=1
    ...
    for _wid, _is_pass in zip(rt_wids, rt_pass):
        if _wid:
            ...
            # 1) If this WXY already had a PASS in an earlier RETEST, then this row is
            #    a RETEST duplicate, regardless of the current row PASS/FAIL result.
            # 2) Otherwise, if this WXY has a FIRST PASS PASS original and the current
            #    chain-head is PASS, this is the first duplicate against FIRST PASS and
            #    is counted in the original duplicate counter.
            # 3) If this RETEST row is PASS, it becomes/keeps the RETEST PASS reference
            #    for future RETEST duplicate detection.
    ...
    for _row, _hit, _rt_hit, _wid in zip(retest_rows, _rt_dup_hits, _rt_retest_dup_hits, rt_wids):
        _row[_dl] = _row[_sf]
        _row[_dw] = (_id_to_wxy[_wid], _hit) if _hit else "N/A"
        _row["is_duplicate"] = "YES" if _hit else "NO"
        _row["retest_duplicate_wxy"] = (_id_to_wxy[_wid], _rt_hit) if _rt_hit else "N/A"
        _row["is_retest_duplicate"] = "YES" if _rt_hit else "NO"
        _row["is_total_duplicate"] = "YES" if (_hit or _rt_hit) else "NO"
        _row["reject_in_retest"] = "N/A"
    ...
    _emit_log("Duplicate GUID rule: the first RETEST duplicate against an established FIRST PASS PASS original is counted here.", True, logger)
